...
- ERD 수정 (Menu테이블에 음식타입개수 추가)
- 가게리스트 조회 api sort 순서 수정 
- 쿼리/API 작성 (메뉴조회, 음식담기조회)
- 휴대폰,이메일중복체크 api 수정 
- 우리동네 빠른배달 쿼리/api 작성중


## 2021-08-23 진행상황


- 가게메뉴 조회api foodTypeTxt 추가 
- 쿼리/API 작성 (우리동네 빠른배달조회, 배민1 추천 조회, 포장/방문 추천 조회, 가게쿠폰 조회)
- 가게리뷰 올리기 api 작성중
...
- 중복데이터 수정, 주문 더미데이터 추가 
- 쿼리작성 (가게 리뷰올리기, 사장님댓글 올리기, 리뷰조회, 주문하기페이지 조회, 주문하기)
- 가게리뷰, 주문하기 api 작성중
...
- 주문하기 쿼리 수정 
- API 작성 (가게 리뷰올리기, 사장님댓글 올리기, 리뷰조회, 주문하기페이지 조회, 주문하기)


- 개발 이슈
  - 코드는 문제가 없는 것 같은데 계속 데이터베이스 연결에 실패했다고 뜸 
    - 해결: DB에 요청한 데이터 중 일부가 빠져있어서 추가함


## 2021-08-26 진행상황


- 사용자 api, 가게 포스터 사진 수정 
- 쿼리/API 작성 (주문하기, 주문상태 확인, 최근 주문, 찜하기, 찜 조회, 주문내역 조회, 네이버 소셜로그인) 
- 주문상세 조회 쿼리 작성